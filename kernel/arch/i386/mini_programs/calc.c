...
void calculator() {
    printf("\t[CALCULATOR]\n\n");
    float a, b; // Изменили тип переменных на float для поддержки чисел с плавающей запятой
    char op;

    printf("Enter first number: ");
    scanf("%f", &a); // Используем %f для считывания числа с плавающей запятой

    printf("Enter operator (+, -, *, /, %%): ");
    scanf(" %c", &op); // Обратите внимание на пробел перед %c, чтобы игнорировать пробелы

    printf("Enter second number: ");
    scanf("%f", &b); // Используем %f для считывания числа с плавающей запятой

    float result; // Изменили тип результата на float
    switch (op) {
        case '+':
            result = a + b;
